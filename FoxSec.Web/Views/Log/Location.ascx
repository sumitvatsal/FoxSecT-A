...
<div id='LocContent'>
    <form id="locFilters">
        <table cellpadding="0" cellspacing="1" style="margin: 0; width: 100%; padding: 0; border-spacing: 1px;">
                ...
                <td style="width: 18%; padding-right: 3px">
                    ...
                    <table cellpadding="0" cellspacing="0" style="margin: 0; width: 100%; padding: 0; border-spacing: 0px;">
                        <tr id="fromDateRow">
                            ...
                            <td style="width: 75%;">
                                <%:Html.TextBox("FromDate", string.Format("{0} {1}:{2}", DateTime.Now.AddDays(-1).ToString("dd.MM.yyyy"), DateTime.Now.Hour.ToString("D2"), DateTime.Now.Minute.ToString("D2")))%>
                            </td>
                        </tr>
                        <tr>
                            <td style="width: 25%">
                        ...
                        <tr style="color: red; width: 75%">
                            <td colspan="2" style="color: red; width: 75%">* Date fromat is dd/mm/yyyy HH:mm</td>
                        </tr>
            ...
            <input type='button' id='Button3' style='display:none' value='<%=ViewResources.SharedStrings.ReportToCompany %>' onclick = "ReportCompany()"  /> 
                        <label>     	<%:Html.CheckBox("ischeck",true,new { Style="margin-top:10px;"}) %> Corrected Algorithm</label>
		</td>--%>
                ...
                <td style="width: 25%; vertical-align: top; padding-top: 2px">
                    ...
                    <%} %>
                     <div id="deactivateButton">
                  <!--  <input type="button" id="deactivateUser" value="Deactivate User" onclick="DeactivateUser()" disabled/>-->
                         </div>
                </td>
                <td style="width: 25%; vertical-align: top; padding-top: 2px">
                    <label for='Search_company'><%:ViewResources.SharedStrings.UsersCompany %></label><br />
                ...
                <td style='width: 30%; margin-left: 150px'>
                    <label><%:Html.CheckBox("ischeck",true,new {id="isCheck", Style="margin-top:10px;"}) %> Not Corrected Algorithm</label>
                    <br />
                    <label><%:Html.CheckBox("NotMoved",false,new {id="notMoved", Style="margin-top:10px;"}) %> Not moved after To date</label>
                    <div id='logPrintControlButtons' style="display: none; text-align: right">
                        <a style="cursor: pointer;" onclick="javascript:SetLocExportLink(this,'/Print/LocListPDF',1)">PDF</a> / <a style="cursor: pointer;" onclick="javascript:SetLocExportLink(this,'/Print/LocListExcel',2)">XLS</a>
                    </div>
                </td>
    ...
    <script type="text/javascript" language="javascript">
        ...
        $(document).ready(function () {
            ...
            $("#notMoved").click(function () {
                ...
                if ($("#notMoved").is(":checked")) {
                    ...
                    var dateTime = date + "." + month + "." + year + " 00:00";
                   
                    var dateTimeParsed = Date.parse(dateTime);
                    $('[name="ToDate"]').val(dateTime);
                        $("#fromDateRow").hide();
                        $("#fromDateRow :input").prop("disabled", true);
                    ...
                    var dateTime = date + "." + month + "." + year + " 00:00";
                    $('[name="ToDate"]').val(dateTime);
                        $("#fromDateRow :input").prop("disabled", false);
                        $("#fromDateRow").show();
                    }
            });

            var i = $('#panel_owner li').index($('#LocationTab'));
        ...
        function SetLocExportLink(link, base, type) {
            var rep = $('#ReportId option:selected').text();
            var comp = $('#CompanyId option:selected').text();
