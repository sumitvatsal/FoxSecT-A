                                ...
                                <% foreach (var obj in Model.Objects) { if (Model.User.IsBuildingAdmin || Model.User.IsSuperAdmin) { if (obj.ParentId == floor.MyId) { %>
                                 <ul>
                                    <li id='permissionTreeLi_<%= obj.MyId %>'>
                                        ...
                                        <% if(!obj.IsDefaultTimeZone) {%><td style="width:15px"><span class="ui-icon ui-icon-person"></span></td><% } %>
                                        <td>
											<span <% if (!string.IsNullOrWhiteSpace(obj.Comment)){ %>class = "tipsy_we" original-title="<%= obj.Comment %>" <% } %>>
												&nbsp;<a onclick="javascript:GetZoneByObject(<%= obj.MyId %>,<%= obj.IsRoom %>);" style="color:<%= !string.IsNullOrEmpty(obj.Comment) ? "Green" : "#222222"%>"><%= Html.Encode(obj.Name)%></a>
												<input id='permissionTreeObject_<%= obj.MyId %>' name='<%= obj.MyId %>' onclick="javascript:$('table#permZoneTreeListRoomNode_<%= obj.MyId %>').toggle();CheckRoom(this,<%= obj.IsRoom %>);" type="checkbox" <% if(Model.ActiveObjectIds != null && Model.ActiveObjectIds.Contains(obj.MyId)){ %>checked = "checked" <% } %>/>
											</span>
										</td>
                                        </tr>
                                        </table>
                                ...
                                <ul>
                                    <li id='permissionTreeLi_<%= obj.MyId %>'>
                                        ...
                                        <td>
											<span <% if (!string.IsNullOrWhiteSpace(obj.Comment)){ %>class = "tipsy_we" original-title="<%= obj.Comment %>" <% } %>>
												&nbsp;<a onclick="javascript:GetZoneByObject(<%= obj.MyId %>,<%= obj.IsRoom %>);" style="color:<%= !string.IsNullOrEmpty(obj.Comment) ? "Green" : "#222222"%>"><%= Html.Encode(obj.Name)%></a>
												<input id='permissionTreeObject_<%= obj.MyId %>' name='<%= obj.MyId %>' onclick="javascript:$('table#permZoneTreeListRoomNode_<%= obj.MyId %>').toggle();CheckRoom(this,<%= obj.IsRoom %>);" type="checkbox" <% if(Model.ActiveObjectIds != null && Model.ActiveObjectIds.Contains(obj.MyId) && Model.IsGroupExist ) { %>checked = "checked" <% } %>
