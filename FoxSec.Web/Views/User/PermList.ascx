...
<td style="width: 16%"><input type="checkbox" id='userPermissionTimeZoneCheck_<%= zone.Id %>' <% if(Model.IsModelReadOnly ) { %>disabled = "disabled" <% } %>  name='<%= zone.Id %>' onclick="javascript:ToggleUserPermissionTimeZone(<%= zone.Id %>);" /> <%= Html.Encode(zone.Name) %></td>
