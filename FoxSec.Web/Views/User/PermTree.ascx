                                    ...
                                    <li id='userPermissionTreeLi_<%= obj.MyId %>'>
                                        ...
                                        <td>
											<span <% if(!string.IsNullOrWhiteSpace(obj.Comment)){ %>class="tipsy_we" original-title="<%= obj.Comment %>" <% } %>> 
												<a onclick="javascript:GetUserZoneByObject(<%= obj.MyId %>,<%= obj.IsRoom %>);" style="color:<%= !string.IsNullOrEmpty(obj.Comment) ? "Green" : "#222222"%>"><%= Html.Encode(obj.Name)%></a>
											</span>
											 <input id='userPermissionTreeObject_<%= obj.MyId %>' <% if(Model.IsCurrentUserAssignedGroup ) { %>disabled = "disabled" <% } %>  name='<%= obj.MyId %>' onclick="javascript:CheckRoom(this,<%= obj.IsRoom %>);" type="checkbox" <% if(Model.ActiveObjectIds != null && Model.ActiveObjectIds.Contains(obj.MyId)) { %>checked = "checked" <% } %>/>
                                        </td>
                                        </tr>
                                        </table>
                                        <% if(obj.IsRoom == 1){ %>
